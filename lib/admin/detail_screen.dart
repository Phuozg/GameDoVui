...
class DetailQuestion extends StatefulWidget {
  const DetailQuestion(
      {super.key, required this.questionID, required this.questionContent});
  final questionID;
...
class _DetailQuestionState extends State<DetailQuestion> {
  ...
  Widget build(BuildContext context) {
    ...
    showEditDialog(String contentt) {
      TextEditingController content = TextEditingController(text: contentt);
      showDialog(
          context: context,
          builder: (_) => StatefulBuilder(builder: (context, setState) {
                return AlertDialog(
                  title: const Center(child: Text('Sửa câu hỏi')),
                  content: SizedBox(
                    height: MediaQuery.of(context).size.height / 4,
                    child: Column(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                ...
                                onPressed: () {
                                  setState(() {
                                    Controller().editQuestion(
                                        widget.questionID, content.text);
                                  });
                                  Navigator.of(context).pop();
                                },
                                style: ElevatedButton.styleFrom(
    ...
    showDeleteDialog(String questionID) {
      showDialog(
          context: context,
          builder: (_) => StatefulBuilder(builder: (context, setState) {
                return AlertDialog(
                  title: const Center(child: Text('Xóa trả lời')),
                  content: SizedBox(
                    height: MediaQuery.of(context).size.height / 4,
                    child: Column(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
              ...
              children: [
                Row(
                  ...
                  children: [
                    ElevatedButton(
                        onPressed: () {
                          showEditDialog(widget.questionContent);
                        },
                        style: ElevatedButton.styleFrom(
                            backgroundColor: Colors.blue),
                        child: const Text(
                    ...
                    ElevatedButton(
                        onPressed: () {
                          showDeleteDialog(widget.questionID);
                        },
                        style: ElevatedButton.styleFrom(
                            backgroundColor: Colors.blue),
                        child: const Text(
